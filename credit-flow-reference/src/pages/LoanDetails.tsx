...
import { ArrowLeft, Ban, Calendar, CheckCircle, Clock, DollarSign, FileText, History, List, Loader2, Percent, User, XCircle } from 'lucide-react'
...
export default function LoanDetails() {
  ...
  const generatePaymentSchedule = async (loanId: string, loanData: Loan) => {
    ...
    for (let i = 1; i <= durationMonths; i++) {
      ...
      const principal = i === durationMonths
        ? amount - (principalPerMonth * (durationMonths - 1))
        : principalPerMonth

      const interest = i === durationMonths
                ...
                {loan.validation_date && loan.validator && (
                  <div className="p-3 rounded-lg bg-muted/50">
                    <p className="text-sm text-muted-foreground mb-1">
                      {loan.status === 'approuve' ? 'Approuvé' : 'Rejeté'}
                      {' '}
                      le
                      {formatDate(loan.validation_date)}
                    </p>
                    <p className="text-sm font-medium">
                      par
                      {loan.validator.full_name}
                    </p>
                  </div>
                )}
              </CardContent>
