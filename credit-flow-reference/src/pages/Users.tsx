...
import { AlertTriangle, Ban, CheckCircle, History, Key, Loader2, MoreHorizontal, Plus, Shield, Trash2, UserPlus } from 'lucide-react'
...
export default function Users() {
  ...
  const handleChangePassword = async () => {
    if (!userToChangePassword || !isAdmin || !newPassword)
      return
  ...
  const handleSuspendUser = async () => {
    if (!userToSuspend || !isAdmin || !suspensionDuration)
      return
                              ...
                              {user.suspension
                                ? (
                                    <Badge className="bg-destructive/10 text-destructive border-destructive/20">
                                      ...
                                      Suspendu jusqu'au
                                      {' '}
                                      {format(new Date(user.suspension.suspended_until), 'dd/MM/yyyy HH:mm', { locale: fr })}
                                    </Badge>
                                  )
                                : (
