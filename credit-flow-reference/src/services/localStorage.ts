 ...
 */
function createAllStores(database: IDBDatabase) {
 ...
 */
export async function saveToLocal<T extends { id: string }>(storeName: string, data: T | T[], synced: boolean = true): Promise<void> {
 ...
 */
export async function getFromLocal<T>(storeName: string, id?: string): Promise<T[]> {
  const database = await initLocalDB()
 ...
 */
export async function searchLocal<T>(storeName: string, indexName: string, value: any): Promise<T[]> {
  const database = await initLocalDB()
  const transaction = database.transaction([storeName], 'readonly')
  const store = transaction.objectStore(storeName)
 ...
 */
export async function deleteFromLocal(storeName: string, id: string | string[]): Promise<void> {
  const database = await initLocalDB()
  const transaction = database.transaction([storeName], 'readwrite')
  const store = transaction.objectStore(storeName)
 ...
 */
export async function markAsSynced(storeName: string, id: string | string[]): Promise<void> {
  const database = await initLocalDB()
  const transaction = database.transaction([storeName], 'readwrite')
  const store = transaction.objectStore(storeName)
 ...
 */
export async function cleanupOldData(storeName: string, daysToKeep: number = 30): Promise<void> {
  const database = await initLocalDB()
  const transaction = database.transaction([storeName], 'readwrite')
  const store = transaction.objectStore(storeName)
