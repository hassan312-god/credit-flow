...
function isValidCSSColor(color: string): boolean {
  ...
  if (/^rgba?\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*(?:(,\s*(0|1|0?\.\d+))\s*)?\)$/.test(sanitized))
    return true

  // Allow hsl/hsla: hsl(0,0%,0%), hsla(0,0%,0%,0.5)
  if (/^hsla?\(\s*\d{1,3}\s*,\s*\d{1,3}%\s*,\s*\d{1,3}%\s*(?:(,\s*(0|1|0?\.\d+))\s*)?\)$/.test(sanitized))
    return true

  // Allow CSS custom properties: var(--color-name)
  if (/^var\(--[a-zA-Z0-9-]+\)$/.test(sanitized))
    return true
