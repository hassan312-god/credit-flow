...
export default defineNuxtRouteMiddleware((to) => {
  ...
  if (!isAuthenticated.value) return navigateTo('/auth/login', { replace: true })
  // Ne jamais rediriger vers / quand on est déjà sur / (évite la boucle infinie)
  if (path === '/') return
  if (!role.value) return navigateTo('/', { replace: true })
  if (!canAccessPath(path)) return navigateTo('/', { replace: true })
})
